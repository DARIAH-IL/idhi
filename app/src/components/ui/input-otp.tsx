...
function InputOTPGroup({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'flex items-center rounded-md has-aria-invalid:border-destructive has-aria-invalid:ring-2 has-aria-invalid:ring-destructive/20 dark:has-aria-invalid:border-destructive dark:has-aria-invalid:ring-destructive/40',
        className,
      )}
      {...props}
    />
    ...
    >
      ...
      {hasFakeCaret && (
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div className="h-4 w-px animate-caret-blink bg-foreground duration-1000 dark:bg-primary-foreground" />
        </div>
      )}
    </div>
  )
...
function InputOTPSeparator({ ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      {...props}
    >
      <HugeiconsIcon icon={MinusSignIcon} strokeWidth={2} aria-hidden="true" />
    </div>
  )
}
